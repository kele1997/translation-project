...
所有人都（好吧，不是所有人）知道 [python](https://www.python.org/) 是一门用途广泛，易读，容易入门的编程语言。  
...
众所周知 python 的 lambda 表达式不支持多行代码。但是可以模拟出多行代码。  
...
虽然看起来很奇怪，但是上面的函数可以使用 **lambda** 表达式函数代替：   
...
**注意** 这不仅对当前的实例，而且对整个类都有影响（如果要精准到与类绑定的函数），并且所有其他的实例也会受到影响：   
...
把可变对象作为函数参数的默认值是非常危险的一件事，并且在面试中有大量关于这方面棘手的问题。但这一点对于缓存机制非常有效。
